...
def get_sys_rating(cur):
    """Rating interno basato sugli indicatori già calcolati."""
    score     = cur["score"]
    er        = cur["er"]
    ao        = cur["ao"]
    ...
    # Conta condizioni bull
    bull = sum([sar_bull, vortex_b, rvi_b, ao > 0, above, trend == "VERDE"])

    if score >= 60 and bull >= 5:
        rating = "STRONG_BUY"
    elif score >= 45 and bull >= 4:
        rating = "BUY"
    elif score >= 30 and above:
        rating = "NEUTRAL"
    elif trend == "ROSSO" and not above and score < 20:
        rating = "STRONG_SELL"
    else:
        rating = "SELL"
...
def analyze(ticker,name):
    try:
        ...
        # Rating interno
        cur_data.update(get_sys_rating(cur_data))
        return cur_data
